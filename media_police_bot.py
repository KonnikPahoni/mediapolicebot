...
SERVER_TIME_DIFFERENCE = UTC_TIME_DIFFERENCE - int(
    dt.datetime.utcnow().astimezone().utcoffset().total_seconds() / 60 / 60)  # Local time ahead of server time

NULLED_DATA_JSON = [[], {"Понедельник": [], "Вторник": [], "Среда": [], "Четверг": [],
...
SELECT_DAY, ASSIGN_MANAGERS, THANK_YOU, THANK_YOU_OPTOUT, SELECT_DAY_UNASSIGN, UNASSIGN_MANAGERS = range(
...
class PoliceBotData:  # singleton for the bot
    ...
    def __init__(self, oathed_dict, assigned_dict):
        ...
        try:
            self.post_for_today, self.post_for_tomorrow, self.last_shared_fb_like = pickle.load(
                open("bot_data/persistence.p", "rb"))
        except:
    ...
    def persistence(self):  # saving variables
        pickle.dump((self.post_for_today, self.post_for_tomorrow, self.last_shared_fb_like),
    ...
    def str_assigned(self, day, mentioning=False):  # assigned users as a string (with mentioning)
        result_text = ''
    ...
    def save(self):
        with open(self.ASSOCIATED_FILENAME, "w", encoding="utf-8") as data_json:
            data_json.write(json.dumps([self.oathed, self.assigned], ensure_ascii=False))
    ...
    if update.message.from_user in chat_data:
        ...
        context.bot.send_message(
            ...
            reply_markup=telegram.ReplyKeyboardMarkup(
                optin_buttons, resize_keyboard=True, one_time_keyboard=True,
                selective=True,
            )
        )
        return THANK_YOU
...
def thank_you(update, context):
    ...
    context.bot.send_message(chat_id=update.effective_chat.id,
                             text=message_text, reply_to_message_id=update.message.message_id,
...
def initiate_opting_out(update, context):
    if update.message.from_user not in chat_data:
        ...
        context.bot.send_message(chat_id=update.effective_chat.id,
                                 text=message_text,
                                 reply_markup=telegram.ReplyKeyboardMarkup(
                                     optout_buttons, resize_keyboard=True, one_time_keyboard=True, selective=True),
                                 reply_to_message_id=update.message.message_id)
...
def thank_you_optout(update, context):
    if update.message.text == optout_buttons[0][0]:
        message_text = 'Как знаешь. Возможно, ты найдешь себя в другой роли. Отречение принято.'
...
def routine(update, context):
    ...
    context.bot.send_message(chat_id=update.effective_chat.id, text=routine_text,
                             reply_to_message_id=update.message.message_id, parse_mode='HTML')


@selectivity
def oathed(update, context):
    context.bot.send_message(chat_id=update.effective_chat.id, text='Присягнувшие: ' + str(chat_data),
                             reply_to_message_id=update.message.message_id, parse_mode='HTML')


@selectivity
def cancel(update, context):
    context.bot.send_message(chat_id=update.effective_chat.id, text='Все операции остановлены.',
                             reply_markup=remove_keyboard_markup)
    return ConversationHandler.END
...
def initiate_assigning(update, context):
    context.bot.send_message(chat_id=update.effective_chat.id,
                             ...
                             reply_markup=telegram.ReplyKeyboardMarkup(weekdays_buttons,
...
def select_day(update, context):
    ...
    for user in chat_data:
        if user['id'] not in chat_data.assigned[day]:
            ...
            buttons.append(telegram.InlineKeyboardButton(text=button_text, callback_data=str(user['id'])))
    if len(buttons) != 0:
        inline_markup = telegram.InlineKeyboardMarkup(list(zip(buttons)))
        context.bot.send_message(
            chat_id=update.effective_chat.id,
            text='Выбери пользователя, которого нужно добавить на указанный день (' + day.lower() + ').',
...
def select_day_unassign(update, context):
    ...
    if len(chat_data.assigned[day]) != 0:
        ...
        context.bot.send_message(
            chat_id=update.effective_chat.id,
            text='Выбери пользователя, которого нужно удалить с указанного дня (' + day.lower() + ').',
            reply_markup=inline_markup,
            reply_to_message_id=update.message.message_id,
        )
...
def get_scheduled_posts(days_from_today, include_all=False):
    result = []
    selection_date = localize(dt.datetime.today()).date() + dt.timedelta(days=days_from_today)
    posts = graph.get_object(id=str(FACEBOOK_PAGE_ID) + '/scheduled_posts')
    for post_data in posts['data']:
        post_datetime = localize(dt.datetime.fromtimestamp(
            graph.get_object(id=post_data['id'], fields="scheduled_publish_time")['scheduled_publish_time']))
        if (include_all and post_datetime.date() <= selection_date) or (post_datetime.date() == selection_date):
            result.append({'created_time': post_datetime,
    ...
    if not chat_data.post_for_today:
        ...
        if scheduled:
            ...
            if chat_data.assigned[week_day]:
                message_text += '<b>Спасибо, ' + chat_data.str_assigned(week_day) + '</b>'
        elif posted:
            ...
            if chat_data.assigned[week_day]:
                message_text = random.choice(sameday).replace('#', chat_data.str_assigned(week_day,
                                                                                          mentioning=True))
...
def nextday_checker(context):
    if not chat_data.post_for_tomorrow:
        ...
        if scheduled:
            ...
            if chat_data.assigned[week_day]:
                message_text = random.choice(nextday).replace('#', chat_data.str_assigned(week_day,
                                                                                          mentioning=True))
        context.bot.send_message(chat_id=TELEGRAM_CHAT_ID, text=message_text, parse_mode='HTML',
                                 disable_web_page_preview=True)
...
def evening_nuller(context):
    ...
    if not chat_data.post_for_today:
        ...
        if chat_data.assigned[week_day]:
            message_text += ' ' + chat_data.str_assigned(week_day, mentioning=True) + ', как тебе не стыдно!'
        context.bot.send_message(chat_id=TELEGRAM_CHAT_ID, text=message_text, parse_mode='HTML',
                                 disable_web_page_preview=True)
...
assign_handler = ConversationHandler(
    ...
    states={SELECT_DAY: [MessageHandler(Filters.text(WEEKDAYS), select_day),
                         cancel_handler,
...
unassign_handler = ConversationHandler(
    ...
    states={SELECT_DAY_UNASSIGN: [MessageHandler(Filters.text(WEEKDAYS), select_day_unassign),
                                  cancel_handler,
...
optout_handler = ConversationHandler(
    ...
    states={
        THANK_YOU_OPTOUT: [MessageHandler(Filters.text(optout_buttons_captions), thank_you_optout)],
        TIMEOUT: [MessageHandler(Filters.all, cancel)]},
    fallbacks=[cancel_handler,
               MessageHandler(Filters.all, invalid_answer_buttons(THANK_YOU))],
    conversation_timeout=TIMEOUT_TIME)
